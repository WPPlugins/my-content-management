...
";
	...
	<form method='post' action='".admin_url('options-general.php?page=my-content-management/my-content-management.php')."'>
		...
		<p>
		<input type='checkbox' name='has_donated' id='has_donated' value='on' /> <label for='has_donated'>" . sprintf( __( 'I <a href="%s">made a donation to help support this plug-in</a>.', 'my-content-management' ), 'https://www.joedolson.com/donate/' ) . "</label>
		</p>
		<p>
...
function mcm_settings_page() {
	...
	<div class="postbox-container" style="width: 70%">
		...
		<h2 class='hndle'><?php _e('Get Plug-in Support','my-content-management'); ?></h2>
			<div class="inside">
				<div class='mcm-support-me'>
					<p>
						<?php printf(
							__( 'Please, consider a <a href="%s">donation</a> or a <a href="%s">purchase</a> to support My Content Management!', 'my-content-management' )
						, "https://www.joedolson.com/donate/", "https://www.joedolson.com/my-content-management/guide/" ); ?>
					</p>
				</div>	
...
<p>{link_title}</p>", 
	...
	if ( is_array($enabled) ) {
		...
		foreach ( $enabled as $value ) {
			...
			<h3>".sprintf(__('%s Templates', 'my-content-management'),$types[$value][2] )."</h3>
...
function mcm_delete_custom_field_support( $fieldset, $post_type ) {
	...
	if ( in_array( $post_type, $array ) ) { 
		$key = array_search( $post_type, $array ); 
		unset( $array[$key] ); 
	}
	$option['extras'][$fieldset][0] = $array;
	update_option( 'mcm_options', $option );
...
function mcm_get_fieldset( $fieldset=false ) {
	...
	if ( !$fieldset ) {
		foreach ( $posts as $value ) {
			...
			if ( $fieldset ) {
				$checked = ( in_array( $name, $types ) ) ? ' checked="checked"' : '';
			} else {
				$checked = '';
			}
			$post_types .= "<input type='checkbox' value='$name' name='mcm_assign_to' id='mcm_post_type_$name'> <label for='mcm_post_type_$name'>$label</label>\n";
		}
		$fieldset_title = "
			<p>
...
function mcm_update_custom_fieldset( $post ) {
	...
	if ( isset( $post['mcm_new_fieldset'] ) ) { 
		...
		$mcm_assign_to = $option['extras'][$fieldset][0];
